...
<script>
    ...
    .then(restData => {
        ...
        paginationPage("buttonPage","btnPageComment","viewComment",'data',totalPage,start,end);
    ...
    const viewComment = (e,start,end) => {
        let viewProduct = e.slice(start,end).map(e => `
            <tr class="bg-slate-800 ">
                ...
                <th class="w-[5%] smr:w-[10%] text-white text-[15px] font-semibold border-solid border-white border-[2px] px-6 py-3">${e.idUser}</th>
                <th class="w-[10%] text-white text-[15px] font-semibold border-solid -border-white border-[.2px] px-6 py-3">${e.commentValue}</th>
